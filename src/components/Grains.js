...
const SortableList = SortableContainer(({grains, collection, handleChange}) => {
    return (
        <tbody>
            {grains.map((attr, index) => {
...
const Grains = ({grains, title, handleClick, onSortEnd, handleChange}) => {
    return (
       <Panel header={`${title}s` || "no title"}>
            <Table striped bordered condensed hover>
                ...
                <SortableList grains={grains} collection={title} onSortEnd={onSortEnd} handleChange={handleChange} />
            </Table>
